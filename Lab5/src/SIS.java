...
public class SIS {
    ...
    //the backend contains the data bases
    ...
    private void enrollStudent(String username, int courseId){
        if (!verifyStudentCourse(username,courseId)){backend.enrollStudent(username,courseId);}
    }

    //A helper method that list all courses.
    ...
    private void listCourse(int courseId){
        System.out.println(backend.getCourse(courseId).toString());
    }

    //A helper method for the professor and student commands.
    private void listUser(String username){
        Collection<Course> courses = backend.getCoursesUser(username);
        for (Course c : courses){
            System.out.println(c.toString());
        }
    ...
    private void unenrollStudent(String username, int courseId){
        if (verifyStudentCourse(username,courseId)){backend.unenrollStudent(username, courseId);}
    }
    ...
    private boolean	verifyStudentCourse(String username, int courseId){
        return backend.getCoursesUser(username).contains(backend.getCourse(courseId));
    }
